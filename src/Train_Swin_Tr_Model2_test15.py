...
hyper_parameter_group ={
    ...
    "seed": 2021,
    "out_dir": r'../model/Swin_Tr_224/test15',
    "log_file_name": "Swin_Tr_test15",
    ...
    "optimizer_type": "Adam" , # 支持 AdamW RAdam Adam 三种
...
class Swin_Tr_224_Model1_large_test15(nn.Module):
    def __init__(self, pretrained=False):
        super().__init__()
        self.encoder = timm.create_model(hyper_parameter_group["encoder_name"], pretrained=pretrained)
        # 删除掉最后的 head头
        ...
        for name, param in self.encoder.named_parameters():
            # 将 嵌入层 以及 0 1 2 层全部冻结
            if param.requires_grad and 'patch_embed' in name:
                param.requires_grad = False
            if param.requires_grad and 'layers.0' in name:
                param.requires_grad = False
            if param.requires_grad and 'layers.1' in name:
                param.requires_grad = False
            if param.requires_grad and 'layers.2' in name:
                param.requires_grad = False
        self.encoder.head = nn.Linear(self.n_features, 128)
        self.fc1 = nn.Linear(128, hyper_parameter_group["output_dims"])
...
##########################################################如上代码 生成 对应的 10_fold csv文件 便于云端进行 svr boost 等一系列训练
    ...
    # 定义模型
    model = Swin_Tr_224_Model1_large_test15(pretrained=True)
    ...
    # 每 iter_save 次保存
    iter_save = 32   #  fROM https://www.kaggle.com/c/petfinder-pawpularity-score/discussion/289889  EVRY 32 ITERATIONS  run once
    # 最大循环步长 每10epoch修正一次
