...
hyper_parameter_group ={
    # 主要环境参数设定
    "seed": RANDOMSEED,
    "out_dir": r"/storage/Kaggle_Pet_Finder/model/Swin_Tr_224/fastai_test10",
    "log_file_name": "Fastai_SwinTr_test10",
    # 数据集设定
...
def get_data(fold):
    ...
    dls = ImageDataLoaders.from_df(train_df_f, #pass in train DataFrame
                            ...
                            #    label_col='norm_score', #label is in the first column of the DataFrame
                                label_col=hyper_parameter_group["dense_features"],
...
class Swin_Tr_384_Model1_large_test10(nn.Module):
    def __init__(self, pretrained=False):
        super().__init__()
        self.encoder = create_model(hyper_parameter_group["encoder_name"], pretrained=True)
        # 删除掉最后的 head头
    ...
    def forward(self, input, input2):
        ...
        x1_outdrop = self.dropout1(x1)
        x = torch.cat([x1_outdrop, input2[::,1:]], dim=1)
        x = self.fc1(x)
...
# 多维输出 需要hack 一下 learner 中的 do_one_batch方法

def _do_one_batch_custom(self):
    # 2022年1月6日 更新：
    self.pred = self.model(*self.xb,*self.yb)["output"]
    # self.pred = self.model(*self.xb)
    self('after_pred')
    if len(self.yb):
        # print(len(self.yb))
        self.loss_grad = self.loss_func(self.pred, self.yb[0][::,:1])
...
# hack 加速values 中的 targs方法
def accum_values_custom(self, preds, targs,learn=None):
    "Store targs and preds"
    to_d = learn.to_detach if learn is not None else to_detach
    preds,targs = to_d(preds),to_d(targs[::,:1])
    if self.flatten: preds,targs = flatten_check(preds,targs)
    self.preds.append(preds)
    self.targs.append(targs)

Learner._do_one_batch = _do_one_batch_custom
AccumMetric.accum_values = accum_values_custom


def get_learner(fold_num):
    data = get_data(fold_num)
    
    # 2022年1月6日 更新 使用 原 test9中的结构
    

    # model = create_model(hyper_parameter_group["encoder_name"], pretrained=True, num_classes=data.c)
    model = Swin_Tr_384_Model1_large_test10()

    learn = Learner(data, model, 
    path = hyper_parameter_group["out_dir"],
    loss_func=BCEWithLogitsLossFlat(), 
    metrics= [AccumMetric(func=petfinder_rmse)]).to_fp16()
...
# 2022年1月6日 更新  换用 YOLOV5 预处理之后的
# train_df['path'] = train_df['Id'].map(lambda x:str(dataset_path/'clean_data_onlypets'/ (str(x)+'_petonly') )+'.jpg')
