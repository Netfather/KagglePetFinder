...
hyper_parameter_group ={
    # 主要环境参数设定
    "seed": 3407,
    "out_dir": r'../model/Swin_Tr_224/test6',
    "log_file_name": "Swin_Tr_test6",
    ...
    # test 3 5 swin_large_patch4_window12_384_in22k
    # test 4  swin_large_patch4_window7_224
...
if hyper_parameter_group["no_log"] == False:
    ...
    stdout('\n')
    copyfile("Train_Swin_Tr_Model2_test6.py", hyper_parameter_group["out_dir"] + "/source_code/train_source_code.py")
...
train_trfm = A.Compose([
    ...
    A.OneOf([
        A.CoarseDropout(max_holes=8, max_height=8, max_width=8, fill_value=0, p=1.0),
        A.RandomResizedCrop(height= hyper_parameter_group["image_resize_resolution"],width = hyper_parameter_group["image_resize_resolution"],
                            scale= (0.7,1.0) , ratio= (0.8,1.2), p = 1.0),
    ], p=0.40),
...
val_trfm = train_trfm  # 为了cv更好的拟合 lb 做出的修正
...
def validation(model, loader):
    ...
    for data in loader:
        ...
        if hyper_parameter_group["loss_type"] != "mser":
            msre_meric = mser_with_logits_numpy(predict, truth)
            valid_loss.append(msre_meric)
        else:
            msre_meric = mser_numpy_only_for_mserloss(predict, truth)
            valid_loss.append(msre_meric)
    ...
    del image,target,output,msre_meric
    gc.collect()
    return [np.array(valid_loss).mean(),0]
...
for fold,(train_idx,val_idx) in enumerate(skf.split(total_df['Id'],total_df['Pawpularity'])):
    ...
    val_dataset = PetfinderDataset(val_df,transform= train_trfm)
    ...
    # 定义模型
    model = Swin_Tr_384_Model2_test6(pretrained=True)
    ...
    while epoch <= hyper_parameter_group["total_epoch"]:
