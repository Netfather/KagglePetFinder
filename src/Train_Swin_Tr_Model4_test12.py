...
hyper_parameter_group ={
    ...
    "seed": 3407,
    "out_dir": r'../model/Swin_Tr_224/test12',
    "log_file_name": "Swin_Tr_test12",
    ...
    "encoder_name" : "swin_large_patch4_window7_224_in22k",
    "output_dims": 1,
    # 学习率计划 优化器 相关
    "total_epoch" : 10,
    "batch_size" : 32,
...
if hyper_parameter_group["no_log"] == False:
    ...
    stdout('\n')
    copyfile("Train_Swin_Tr_Model4_test12.py", hyper_parameter_group["out_dir"] + "/source_code/train_source_code.py")
...
##########################################################如上代码 生成 对应的 10_fold csv文件 便于云端进行 svr boost 等一系列训练
    ...
    # 定义模型
    model = Swin_Tr_224_Model4_large_22k_test12(pretrained=True)
