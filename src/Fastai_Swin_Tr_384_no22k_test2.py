...
# 2022年1月4日： Test2
# 1. 使用 fastai下的 dp 框架 
# 2. 模型使用最原始的 swin_tr 不加入任何修改 使用 384_no22k模型
###############
import os
...
hyper_parameter_group ={
    ...
    "seed": RANDOMSEED,
    "out_dir": r"/storage/Kaggle_Pet_Finder/model/Swin_Tr_224/fastai_test2",
    "log_file_name": "Fastai_SwinTr_test2",
    # 数据集设定
...
# 定义模型结构
def get_learner(fold_num,train_df):
    data = get_data(fold_num,train_df)
    
    model = create_model(hyper_parameter_group["encoder_name"], pretrained=True, num_classes=data.c)

    learn = Learner(data, model, path = 
                    os.path.join(hyper_parameter_group["out_dir"],"checkpoint"),
...
for i in range(hyper_parameter_group["skf_fold"]):
    ...
    learn = get_learner(fold_num=i,train_df = train_df)
    learn.model = torch.nn.DataParallel(learn.model)
    # 测试 最优学习率
    # print(learn.lr_find(end_lr=5e-2))
    # learn.recorder.plot_loss()
    # plt.savefig(os.path.join(logdir,'lr_chose_fold{}.png'.format(i+1))) # 保存训练过程中的 plot图片
