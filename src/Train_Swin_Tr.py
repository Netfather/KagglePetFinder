...
hyper_parameter_group ={
    # 主要环境参数设定
    "seed": 19970711,
    "out_dir": r'../model/Swin_Tr_224/test2',
    ...
    "total_epoch" : 20,
    "batch_size" : 64,
    "cycle_epoch" : 20, #这个参数是指 多少次重启 由于是 迁移学习 因此 这个参数和总训练轮数保持一致即可
    "start_lr" : 1e-5,
    "lr_max_cosin" : 1e-5,
    "lr_min_cosin": 1e-6,
    "lr_gammar_cos": 1.0,
...
if hyper_parameter_group["no_log"]:
    ...
    logger = get_logger(logdir,OutputOnConsole = True,log_initial= "Swin_Tr_test1",logfilename="Swin_Tr_test1")
    stdout = logger.info
...
if hyper_parameter_group["no_log"] == False:
    ...
    stdout('\n')
    copyfile("Train_Swin_Tr.py", hyper_parameter_group["out_dir"] + "/source_code/train_source_code.py")
...
for fold,(train_idx,val_idx) in enumerate(skf.split(total_df['Id'],total_df['Pawpularity'])):
    ...
    meta_iteration_per_epoch = int(len(train_dataset) / hyper_parameter_group["batch_size"])
