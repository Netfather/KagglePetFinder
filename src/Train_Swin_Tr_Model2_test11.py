...
if hyper_parameter_group["no_log"] == False:
    ...
    stdout('\n')
    copyfile("Train_Swin_Tr_Model2_test11.py", hyper_parameter_group["out_dir"] + "/source_code/train_source_code.py")
...
##########################################################如上代码 生成 对应的 10_fold csv文件 便于云端进行 svr boost 等一系列训练
    ...
    # 定义模型
    model = Swin_Tr_384_Model2_large_22k_test11(pretrained=True)
