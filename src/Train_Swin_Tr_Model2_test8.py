...
hyper_parameter_group ={
    ...
    # test8 为了避免过拟合 更换
    "seed": 2021,
    "out_dir": r'../model/Swin_Tr_224/test8',
    "log_file_name": "Swin_Tr_test8",
    ...
    "encoder_name" : "swin_base_patch4_window12_384_in22k",
    "output_dims": 1,
    # 学习率计划 优化器 相关
    "total_epoch" : 10,
    "batch_size" : 54,
...
if hyper_parameter_group["no_log"] == False:
    ...
    stdout('\n')
    copyfile("Train_Swin_Tr_Model2_test8.py", hyper_parameter_group["out_dir"] + "/source_code/train_source_code.py")
...
for fold,(train_idx,val_idx) in enumerate(skf.split(total_df['Id'],total_df['Pawpularity'])):
    ...
    # 定义模型
    model = Swin_Tr_384_Model2_base_test8(pretrained=True)
