...
hyper_parameter_group ={
    ...
    "seed": 3407,
    "out_dir": r'../model/Swin_Tr_224/test9',
    "log_file_name": "Swin_Tr_test9",
...
if hyper_parameter_group["no_log"] == False:
    ...
    stdout('\n')
    copyfile("Train_Swin_Tr_Model1_test9.py", hyper_parameter_group["out_dir"] + "/source_code/train_source_code.py")
...
##########################################################如上代码 生成 对应的 10_fold csv文件 便于云端进行 svr boost 等一系列训练
    ...
    # 定义模型
    model = Swin_Tr_224_Model1_large_test9(pretrained=True)
