...
# -------------------------------------------------------------------------------
# Name:         Train_Swin_Tr
# Description:  此文件用于 实现10 fold 的 baseline 实现
...
hyper_parameter_group ={
    # 主要环境参数设定
    "seed": 3407,
    "out_dir": r'../model/Swin_Tr_224/test5',
    "log_file_name": "Swin_Tr_test5",
...
if hyper_parameter_group["no_log"] == False:
    ...
    stdout('\n')
    copyfile("Train_Swin_Tr_Model2_test5.py", hyper_parameter_group["out_dir"] + "/source_code/train_source_code.py")
...
for fold,(train_idx,val_idx) in enumerate(skf.split(total_df['Id'],total_df['Pawpularity'])):
    ...
    # 定义模型
    model = Swin_Tr_384_Model2_test5(pretrained=True)
